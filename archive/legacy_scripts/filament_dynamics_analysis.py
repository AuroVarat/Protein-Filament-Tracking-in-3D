...
def compute_line_fit_metrics(coords_um: np.ndarray) -> dict[str, float | str]:
    ...
    return {
        ...
        "rod_fit_diameter_um": float(2.0 * rod_fit_rmse_um),
                ...
                frame_records.append(
                    {
                        ...
                        "rod_fit_diameter_um": float(line_fit["rod_fit_diameter_um"]),
        ...
        records.append(
            {
                ...
                "mean_rod_fit_diameter_um": float(group["rod_fit_diameter_um"].mean()),
...
def report_best_rod_fits(summary: pd.DataFrame, top_n: int = 5) -> None:
    ...
    for idx, row in enumerate(best.itertuples(index=False), start=1):
        print(
            ...
            f"diameter ~{row.mean_rod_fit_diameter_um:.3f} um, "
...
def save_rod_fit_examples(
    ...
    required_summary = {"video", "filament_id", "mean_linearity_r2", "mean_rod_fit_cv"}
    required_frames = {
        "video",
        "filament_id",
        "frame",
    ...
    if summary.empty or frame_df.empty:
        return
    if not required_summary.issubset(summary.columns) or not required_frames.issubset(frame_df.columns):
        return

    high = (
        summary.sort_values(["mean_linearity_r2", "mean_rod_fit_cv"], ascending=[False, True])
        .head(examples_per_group)
        .assign(example_group="High rod fit")
    )
    low = (
        summary.sort_values(["mean_linearity_r2", "mean_rod_fit_cv"], ascending=[True, False])
        .head(examples_per_group)
        .assign(example_group="Low rod fit")
    )
    ...
    n_rows = len(examples)
    fig, axes = plt.subplots(n_rows, 2, figsize=(10, max(3 * n_rows, 6)), squeeze=False)

    for row_idx, row in enumerate(examples.itertuples(index=False)):
        video = str(row.video)
        filament_id = int(row.filament_id)
        frame_group = frame_df.loc[
            (frame_df["video"] == video) & (frame_df["filament_id"] == filament_id)
        ].sort_values(["robust_length_um", "linearity_r2"], ascending=[False, False])
        if frame_group.empty:
            for ax in axes[row_idx]:
                ax.axis("off")
            continue
        frame_row = frame_group.iloc[0]
        frame_idx = int(frame_row["frame"])

        if video not in cache:
            mask_stack = tifffile.imread(mask_dir / f"{video}_mask.tif")
            cache[video] = mask_stack
        mask_stack = cache[video]

        frame_mask = mask_stack[frame_idx] == filament_id
        xy_mask = frame_mask.max(axis=0)
        xz_mask = frame_mask.max(axis=1)

        axis_xyz = np.array(
            [
                float(frame_row["principal_axis_x"]),
                float(frame_row["principal_axis_y"]),
                float(frame_row["principal_axis_z"]),
            ],
            dtype=np.float64,
        )
        center_xyz_um = np.array(
            [
                float(frame_row["centroid_x_um"]),
                float(frame_row["centroid_y_um"]),
                float(frame_row["centroid_z_um"]),
            ],
            dtype=np.float64,
        )
        half_len_um = max(float(frame_row["span_length_um"]) / 2.0, 0.3)
        endpoints_xyz_um = np.vstack(
            [
                center_xyz_um - axis_xyz * half_len_um,
                center_xyz_um + axis_xyz * half_len_um,
            ]
        )

        xy_line = np.column_stack(
            [endpoints_xyz_um[:, 0] / pixel_size_xy, endpoints_xyz_um[:, 1] / pixel_size_xy]
        )
        xz_line = np.column_stack(
            [endpoints_xyz_um[:, 0] / pixel_size_xy, endpoints_xyz_um[:, 2] / z_spacing_um]
        )

        xy_ax = axes[row_idx, 0]
        xz_ax = axes[row_idx, 1]
        for ax, mask2d, line2d, title in [
            (xy_ax, xy_mask, xy_line, "XY Mask Projection"),
            (xz_ax, xz_mask, xz_line, "XZ Mask Projection"),
        ]:
            ax.imshow(mask2d.astype(float), cmap="gray", origin="upper", vmin=0.0, vmax=1.0)
            ax.contour(mask2d.astype(float), levels=[0.5], colors=["#FF4D4F"], linewidths=1.0)
            ax.plot(line2d[:, 0], line2d[:, 1], color="#00E5FF", linewidth=2.0)
        ...
        xy_ax.set_ylabel(
            f"{row.example_group}\n{video}#{filament_id}\n"
            f"R2={row.mean_linearity_r2:.2f}, CV={row.mean_rod_fit_cv:.2f}\n"
            f"diam~{row.mean_rod_fit_diameter_um:.2f} um",
            fontsize=9,
        )
...
def main() -> None:
    ...
    save_rod_fit_examples(
        summary=summary,
        frame_df=filtered_frames,
        mask_dir=mask_dir,
        pixel_size_xy=args.pixel_size_xy,
        z_spacing_um=args.z_spacing_um,
        output_path=Path(args.plot_output_dir) / "rod_fit_examples.png",
    )
