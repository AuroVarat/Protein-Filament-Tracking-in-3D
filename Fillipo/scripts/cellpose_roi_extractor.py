...
class CellposeRoiExtractor:
    ...
    def extract_crops(self, hyperstack_5d, bf_channel=0, centers=None):
        """
    ...
    def visualize_clusters(self, input_path, output_plot="cluster_visualization_dbscan.png", data=None):
        ...
        for ax, img, title in zip([ax1, ax2], [ref_bf, ref_other], ["Brightfield", f"Channel {other_channel}"]):
            ...
            if final_centers:
                ...
                ax.scatter(f_centers[:, 1], f_centers[:, 0], c='green', marker='x', s=100, label='ROI Center')
                
                half = self.crop_size // 2
                for i, (y, x) in enumerate(final_centers):
                    rect = patches.Rectangle((x-half, y-half), self.crop_size, self.crop_size, 
                                             linewidth=2.5, edgecolor='green', facecolor='none', alpha=0.8)
                    ax.add_patch(rect)
                    ax.text(x + half, y - half, str(i+1), color='white', backgroundcolor='green', 
    ...
    def extract_and_save(self, input_path, output_dir, data=None, axes=None, centers=None):
        ...
        if data is None:
            ...
            axes = 'TZCYX' # Generic fallback
        
        print(f"Processing: {input_path}")
        crops, extracted_centers = self.extract_crops(data, centers=centers)
